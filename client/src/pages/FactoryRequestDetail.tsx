...
export default function FactoryRequestDetail() {
  ...
  const { data: request, isLoading } = trpc.inquiries.getByBuyer.useQuery({ buyerId: 0 }); // This is a placeholder, in real app we'd have a getById
  ...
  const submitQuote = trpc.inquiries.submitQuote.useMutation({
    onSuccess: () => {
      toast.success(language === 'ar' ? 'تم إرسال العرض بنجاح!' : 'Quote submitted successfully!');
  ...
  return (
    <FactoryDashboardLayout>
      <div className="space-y-6" dir={dir}>
        <Button variant="ghost" onClick={() => setLocation("/factory/orders")} className="gap-2">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <Label className="text-gray-500">{language === 'ar' ? 'المنتج' : 'Product'}</Label>
                    <p className="font-medium">Cotton T-shirts</p>
                  </div>
                  <div>
                    <Label className="text-gray-500">{language === 'ar' ? 'الكمية' : 'Quantity'}</Label>
                    <p className="font-medium">1000</p>
                  </div>
                </div>
                <div>
                  <Label className="text-gray-500">{language === 'ar' ? 'المواصفات' : 'Specifications'}</Label>
                  <p className="bg-gray-50 p-3 rounded-md text-sm">100% Organic Cotton, White, Various Sizes</p>
                </div>
                <div>
                  <Label className="text-gray-500">{language === 'ar' ? 'تفاصيل التوصيل' : 'Delivery Details'}</Label>
                  <p className="bg-gray-50 p-3 rounded-md text-sm">Shipping to Dubai, UAE. Sea freight preferred.</p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <div>
                    <Label htmlFor="price">{language === 'ar' ? 'السعر الإجمالي ($)' : 'Total Price ($)'}</Label>
                    <Input 
                      id="price"
                      type="number"
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {language === 'ar' ? 'سيتم تطبيق عمولة 3% عند قبول الطلب.' : 'A 3% commission will be applied upon acceptance.'}
                    </p>
                  </div>
                  <div>
                    <Label htmlFor="terms">{language === 'ar' ? 'الشروط والملاحظات' : 'Terms & Notes'}</Label>
                    <Textarea 
                      id="terms"
