      ...
      try {
        ...
        const itemsWithDetails = await Promise.all(
          (cartItems || []).map(async (item: any) => {
            ...
            const factory = product ? await db.getFactoryById(product.factoryId) : null;
            return {
              ...item,
              product,
              factory,
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to fetch cart items" });
      }
    }),

  // Add item to cart
  addItem: protectedProcedure
    .input(z.object({ productId: z.number(), quantity: z.number().min(1).max(999) }))
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to add item to cart" });
      }
    }),
  ...
  updateQuantity: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      try {
        ...
        if (error.code) throw error;
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to update quantity" });
      }
    }),
  ...
  clear: protectedProcedure
    .mutation(async ({ ctx }) => {
      try {
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to clear cart" });
      }
    }),
  ...
  getSummary: protectedProcedure
    .query(async ({ ctx }) => {
      try {
        ...
        throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Failed to get cart summary" });
      }
    }),
