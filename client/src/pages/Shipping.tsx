...
export default function Shipping() {
  ...
  const shippingMethods = [
    {
      ...
      description: language === "ar" ? "الخيار الأكثر اقتصادية للكميات الكبيرة" : "Most economical for large volumes",
    ...
    },
    {
      ...
      description: language === "ar" ? "سريع وموثوق للشحنات العاجلة" : "Fast and reliable for urgent shipments",
    ...
    },
    {
      ...
      description: language === "ar" ? "توازن مثالي بين التكلفة والسرعة" : "Perfect balance between cost and speed",
    ...
    },
    {
      ...
      description: language === "ar" ? "توصيل من الباب إلى الباب للدول المجاورة" : "Door-to-door delivery for neighboring countries",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedMethod) {
      toast.error(language === "ar" ? "يرجى اختيار طريقة الشحن" : "Please select a shipping method");
      return;
    }
    ...
    setSubmitted(true);
    toast.success(language === "ar" ? "تم إرسال طلب عرض السعر بنجاح" : "Quote request submitted successfully");
  };

  if (submitted) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4" dir={dir}>
      ...
      <section className="bg-[#1e3a5f] text-white py-16 px-4">
        <div className="max-w-7xl mx-auto text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-6">
            {language === "ar" ? "خدمات الشحن الدولي من الصين" : "International Shipping Services from China"}
          </h1>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            ...
            <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-full">
              ...
              <span>{language === "ar" ? "تتبع لحظي" : "Real-time Tracking"}</span>
            </div>
            <div className="flex items-center gap-2 bg-white/10 px-4 py-2 rounded-full">
              <Globe className="w-5 h-5 text-blue-400" />
              <span>{language === "ar" ? "تغطية عالمية" : "Global Coverage"}</span>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="grid lg:grid-cols-3 gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            <h2 className="text-2xl font-bold text-gray-900">
              {language === "ar" ? "1. اختر طريقة الشحن" : "1. Select Shipping Method"}
            </h2>
              ...
              {shippingMethods.map((method) => (
                ...
                >
                  <CardContent className="p-6">
                    <div className={`w-12 h-12 ${method.color} text-white rounded-lg flex items-center justify-center mb-4`}>
              ...
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="grid sm:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    <Label>{language === "ar" ? "من (المنشأ)" : "From (Origin)"}</Label>
                    <Input value={formData.origin} disabled className="bg-gray-50" />
                  </div>
                  <div className="space-y-2">
                    <Label>{language === "ar" ? "إلى (الوجهة)" : "To (Destination)"}</Label>
                    <Input 
                  ...
                  <div className="space-y-2">
                    <Label>{language === "ar" ? "الوزن التقديري (كجم)" : "Estimated Weight (kg)"}</Label>
                    <Input 
                      type="number"
                      placeholder="0"
                  ...
                  <div className="space-y-2">
                    <Label>{language === "ar" ? "الحجم التقديري (CBM)" : "Estimated Volume (CBM)"}</Label>
                    <Input 
                      type="number"
                      step="0.1"
                      placeholder="0.0"
                ...
                <div className="space-y-2">
                  ...
                  <Input 
                    placeholder={language === "ar" ? "مثال: أثاث، إلكترونيات، ملابس" : "e.g. Furniture, Electronics, Clothing"}
                ...
                <div className="space-y-2">
                  ...
                  <Textarea 
                    placeholder={language === "ar" ? "أي متطلبات خاصة مثل التغليف أو التخليص الجمركي..." : "Any special requirements like packaging or customs clearance..."}
          ...
          <div className="space-y-6">
              ...
              <CardContent className="text-sm text-blue-800 space-y-4">
                ...
                <ul className="space-y-2">
                  <li className="flex items-start gap-2">
                    ...
                    <span>{language === "ar" ? "تخليص جمركي احترافي" : "Professional customs clearance"}</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0" />
                    <span>{language === "ar" ? "تفتيش قبل الشحن" : "Pre-shipment inspection"}</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0" />
                    <span>{language === "ar" ? "تخزين مجاني لمدة 7 أيام" : "Free warehousing for 7 days"}</span>
                  </li>
                </ul>
              </CardContent>
            </Card>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">{language === "ar" ? "أسئلة شائعة" : "FAQ"}</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div>
                  <h4 className="font-bold text-sm mb-1">
                    {language === "ar" ? "كيف يتم حساب التكلفة؟" : "How is cost calculated?"}
                  </h4>
                ...
                <div>
                  <h4 className="font-bold text-sm mb-1">
                    {language === "ar" ? "هل توفرون خدمة التوصيل للباب؟" : "Do you provide door-to-door?"}
                  </h4>
                  <p className="text-xs text-gray-600">
                    {language === "ar" 
