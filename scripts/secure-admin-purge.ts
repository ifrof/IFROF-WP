...
async function secureAdminPurge() {
  ...
  try {
    ...
    if (fs.existsSync(localDbPath)) {
      ...
      console.log(`[Security] Purged ${originalCount - data.users.length} admin accounts from JSON DB.`);
    ...
    try {
      ...
      if (mysqlDb && !mysqlDb.isJsonMode) {
        await mysqlDb.delete(schema.users).where(eq(schema.users.role, 'admin'));
        console.log("[Security] Purged admin accounts from MySQL.");
      }
    } catch (e) {
      console.log("[Security] MySQL purge skipped or failed (likely not configured).");
    }
    ...
    console.log("====================================================\n");
    console.log("IMPORTANT: Save these credentials securely. All other admin accounts have been deleted.");
