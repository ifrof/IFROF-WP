...
export async function generateStaticContent(req: Request): Promise<string> {
  ...
  if (url.startsWith('/products/')) {
    ...
    if (!isNaN(id)) {
      ...
      if (product) {
        const name = lang === 'ar' ? product.nameAr : (lang === 'zh' ? product.nameZh : product.nameEn);
        const desc = lang === 'ar' ? product.descriptionAr : (lang === 'zh' ? product.descriptionZh : product.descriptionEn);
        return `
          <div class="ssr-content">
            <h1>${name}</h1>
            <p>${desc}</p>
