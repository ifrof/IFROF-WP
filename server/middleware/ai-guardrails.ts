...
async function checkRateLimit(
  ...
  if (redis) {
    ...
    if (process.env.NODE_ENV === "production") {
      console.warn(
        "WARNING: Redis not available. Using insecure in-memory rate limiting."
      );
    }
...
export const aiRateLimiter = async (
  ...
  if (userId) {
    ...
    if (!allowed) {
      ...
      return res
        ...
        .json({
          status: "error",
          message: "User rate limit exceeded (10/min).",
          retryAfter,
  ...
  if (!allowed) {
    ...
    return res
      ...
      .json({
        status: "error",
        message: "IP rate limit exceeded (30/min).",
        retryAfter,
        code: "RATE_LIMIT_IP",
      });
  }
...
export const aiDailyCap = async (
  ...
  if (currentCost >= DAILY_CAP_USD) {
    return res
      ...
      .json({
        status: "error",
        message: `Daily AI usage cap of ${DAILY_CAP_USD} USD reached.`,
