...
export default function BuyerRequests() {
  ...
  const { user } = useAuth();
  const { data: requests, isLoading } = trpc.inquiries.getByBuyer.useQuery({ buyerId: user?.id || 0 }, { enabled: !!user });
    ...
    <BuyerDashboardLayout>
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">{language === 'ar' ? 'طلبات الاستيراد' : 'Import Requests'}</h1>
            <p className="text-muted-foreground">
              {language === 'ar' ? 'إدارة طلبات الاستيراد والاستفسارات الخاصة بك.' : 'Manage your import requests and inquiries to factories.'}
            </p>
          </div>
        ...
        {isLoading ? (
          ...
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <CardTitle className="mb-2">{language === 'ar' ? 'لا توجد طلبات' : 'No requests found'}</CardTitle>
              ...
              <Link href="/import-request">
                <Button variant="outline">{language === 'ar' ? 'إنشاء طلب' : 'Create Request'}</Button>
              </Link>
            </CardContent>
          </Card>
        )}
