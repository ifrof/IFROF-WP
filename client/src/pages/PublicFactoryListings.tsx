...
import { Loader2, MapPin, Search, ArrowLeft, ArrowRight, ShieldCheck, Award, ExternalLink } from "lucide-react";
...
export default function PublicFactoryListings() {
  ...
  const filteredFactories = factories?.filter((factory: any) =>
    ...
    factory.productCategories?.toLowerCase().includes(searchQuery.toLowerCase())
  ) || [];
        ...
        <div className="max-w-7xl mx-auto text-center">
          <div className="inline-flex items-center gap-2 bg-blue-500/20 text-blue-200 px-4 py-2 rounded-full mb-6">
            ...
            <span className="font-medium">
              {language === 'ar' ? 'مصانع صينية موثقة' : 'Verified Chinese Factories'}
            </span>
          </div>
          <h1 className="text-4xl md:text-5xl font-bold mb-6">
            {language === 'ar' ? 'دليل المصانع المعتمدة' : 'Certified Factory Directory'}
          </h1>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-muted-foreground">
              {language === 'ar' ? 'جاري تحميل المصانع...' : 'Loading factories...'}
            </p>
          </div>
        ) : filteredFactories.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredFactories.map((factory: any) => (
              <Card key={factory.id} className="overflow-hidden hover:shadow-xl transition-all border-none shadow-md group">
                ...
                <CardContent className="space-y-4">
                  <div className="flex flex-wrap gap-2">
                    {factory.productCategories?.split(',').map((cat: string, i: number) => (
                  ...
                  <p className="text-sm text-gray-600 line-clamp-2 min-h-[2.5rem]">
                    {factory.description || (language === 'ar' ? 'لا يوجد وصف متاح لهذا المصنع حالياً.' : 'No description available for this factory yet.')}
                  </p>
                  ...
                  <div className="pt-4 border-t flex items-center justify-between">
                    <div className="flex items-center gap-1">
                      <div className="flex">
                        {[...Array(5)].map((_, i) => (
                          <span key={i} className={`text-sm ${i < (factory.rating || 4) ? 'text-yellow-400' : 'text-gray-300'}`}>★</span>
                        ))}
                      </div>
      ...
      <section className="bg-white py-16 border-t">
        <div className="max-w-7xl mx-auto px-4 text-center">
          <h2 className="text-3xl font-bold mb-12">
            {language === 'ar' ? 'لماذا تختار المصانع الموثقة؟' : 'Why Choose Verified Factories?'}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="p-6">
              ...
              <h3 className="text-xl font-bold mb-2">{language === 'ar' ? 'أمان مالي' : 'Financial Security'}</h3>
              <p className="text-gray-600">{language === 'ar' ? 'نضمن أن المصنع كيان قانوني مسجل وقائم.' : 'We ensure the factory is a registered and existing legal entity.'}</p>
            </div>
            <div className="p-6">
              <div className="w-12 h-12 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center mx-auto mb-4">
                <Award className="w-6 h-6" />
              </div>
              <h3 className="text-xl font-bold mb-2">{language === 'ar' ? 'جودة مضمونة' : 'Guaranteed Quality'}</h3>
              <p className="text-gray-600">{language === 'ar' ? 'يتم فحص شهادات الجودة (ISO, CE, الخ) والتأكد من صحتها.' : 'Quality certifications (ISO, CE, etc.) are checked and verified.'}</p>
            </div>
            <div className="p-6">
              <div className="w-12 h-12 bg-orange-100 text-orange-600 rounded-lg flex items-center justify-center mx-auto mb-4">
                <MapPin className="w-6 h-6" />
              </div>
              <h3 className="text-xl font-bold mb-2">{language === 'ar' ? 'زيارات ميدانية' : 'On-site Visits'}</h3>
              <p className="text-gray-600">{language === 'ar' ? 'فريقنا في الصين يقوم بزيارات دورية للمصانع المدرجة.' : 'Our team in China conducts periodic visits to listed factories.'}</p>
            </div>
          </div>
        </div>
      </section>
