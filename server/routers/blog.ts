  ...
  create: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const result = await db.insert(blogPosts).values({
        ...
        contentAr: input.contentAr || input.content,
        contentEn: input.contentEn || input.content,
        excerpt: input.excerpt,
        excerptAr: input.excerptAr || input.excerpt,
        excerptEn: input.excerptEn || input.excerpt,
        authorId: ctx.user.id,
        category: input.category,
        categoryAr: input.categoryAr || input.category,
        categoryEn: input.categoryEn || input.category,
        tags: input.tags,
        lang: input.lang,
        published: 0,
      });
